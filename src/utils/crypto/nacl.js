...
const DEFAULT_MAX_CHUNK = 65535;

const ZEROBYTES = nacl.lowlevel.crypto_secretbox_ZEROBYTES;
const BOXZEROBYTES = nacl.lowlevel.crypto_secretbox_BOXZEROBYTES;
const secretbox = nacl.lowlevel.crypto_secretbox;
const secretbox_open = nacl.lowlevel.crypto_secretbox_open;
...
function clean() {
  for (let i = 0; i < arguments.length; i++) {
    ...
    for (let j = 0; j < arg.length; j++) arg[j] = 0;
  }
}
...
StreamEncryptor.prototype.encryptChunk = function (chunk, isLast) {
  ...
  if (chunkLen > this._maxChunkLength)
    throw new Error(
      "chunk is too large: " + chunkLen + " / " + this._maxChunkLength
    );
  for (let i = 0; i < ZEROBYTES; i++) this._in[i] = 0;
  ...
  const encryptedChunk = this._out.subarray(
    BOXZEROBYTES - 4,
    BOXZEROBYTES - 4 + chunkLen + 16 + 4
  );
...
StreamDecryptor.prototype.decryptChunk = function (encryptedChunk, isLast) {
  ...
  const encryptedChunkLen = encryptedChunk.length;
  if (encryptedChunkLen < 4 + BOXZEROBYTES) return this._fail();
  const chunkLen = readChunkLength(encryptedChunk);
  ...
  if (
    secretbox_open(
      this._out,
      this._in,
      encryptedChunkLen + BOXZEROBYTES - 4,
      this._fullNonce,
      this._key
    ) !== 0
  )
