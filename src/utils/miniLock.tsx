...
miniLock.util.validateNonce = function (nonce, expectedLength) {
  ...
  if (base64Match.test(nonce)) {
    const bytes = nacl.util.decodeBase64(nonce);
...
miniLock.util.validateKey = function (key) {
  ...
  if (base64Match.test(key)) {
    const bytes = nacl.util.decodeBase64(key);
...
miniLock.util.isFilenameSuspicious = function (filename) {
  ...
  let extension = filename.toLowerCase().match(/\.\w+$/);
  if (!extension) {
    return true;
  }
...
miniLock.crypto.checkKeyStrength = function (key, email) {
  const minEntropy = miniLock.settings.minKeyEntropy;
  if (key.length < 32) {
    return false;
  }
  if (key.match(email)) {
...
miniLock.crypto.encryptFile = function (
  ...
  const currentFile = miniLock.file.new();
  currentFile.fileName = file.name;
  saveName += ".minilock";
  const fileKey = miniLock.crypto.getFileKey();
  const fileNonce = miniLock.crypto.getNonce().subarray(0, 16);
  currentFile.streamEncryptor = nacl.stream.createEncryptor(
    fileKey,
    fileNonce,
    miniLock.crypto.chunkSize
  );
  const paddedFileName = new Uint8Array(256);
  const fileNameBytes = nacl.util.decodeUTF8(file.name);
...
miniLock.crypto.encryptNextChunk = function (
  ...
  miniLock.file.read(
    ...
    function (chunk) {
      ...
      let encryptedChunk;
      encryptedChunk = currentFile.streamEncryptor.encryptChunk(chunk, isLast);
      if (!encryptedChunk) {
        throw new Error(
          "miniLock: Encryption failed - general encryption error"
        );
        return false;
      }
      currentFile.hashObject.update(encryptedChunk);
      ...
      if (isLast) {
        ...
        const decryptInfoNonces = [];
        for (let u = 0; u < miniLockIDs.length; u++) {
          decryptInfoNonces.push(miniLock.crypto.getNonce());
        }
        for (let i = 0; i < miniLockIDs.length; i++) {
          let decryptInfo = {
            ...
            fileInfo: {
              ...
              fileHash: nacl.util.encodeBase64(currentFile.hashObject.digest()),
            },
          };
          decryptInfo.fileInfo = nacl.util.encodeBase64(
            nacl.box(
          ...
          decryptInfo = nacl.util.encodeBase64(
            nacl.box(
...
miniLock.crypto.decryptFile = function (
  ...
  miniLock.file.read(file, 8, 12, function (headerLength) {
    headerLength = miniLock.util.byteArrayToNumber(headerLength.data);
    miniLock.file.read(file, 12, headerLength + 12, function (header) {
      ...
      let actualDecryptInfo = null;
      let actualDecryptInfoNonce = null;
      let actualFileInfo = null;
      for (const i in header.decryptInfo) {
        if (
      ...
      const dataPosition = 12 + headerLength;
...
miniLock.crypto.decryptNextChunk = function (
  ...
  miniLock.file.read(
    ...
    function (chunk) {
      chunk = chunk.data;
      const actualChunkLength = miniLock.util.byteArrayToNumber(
        chunk.subarray(0, 4)
      );
      ...
      let decryptedChunk;
      let isLast = false;
      if (dataPosition >= file.size - (4 + 16 + actualChunkLength)) {
        isLast = true;
      }
      if (dataPosition === 12 + headerLength) {
        ...
        let fileName = nacl.util.encodeUTF8(decryptedChunk.subarray(0, 256));
