...
const EmojiSuggestions = ({ addSuggestion, chat }) => (
  <ul>
    {chat.suggestions.map((emoj, i) => {
      ...
      const suggestionMD = suggestion.replace(
        /<span class="emoji emoji-sizer" style="background-image:url\((\/static\/img\/emoji\/apple\/64\/.*?)\)" data-codepoints="(?:.*?)"><\/span>/g,
        "![emoji]($1)"
      );
      ...
      return (
        <li {...props} dangerouslySetInnerHTML={{ __html: renderItem }}></li>
      );
    })}
  </ul>
);
