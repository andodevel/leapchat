...
import { PARANOID_USERNAME, USERNAME_KEY } from "../constants/messaging";

const paranoidMode =
  (document && document.location.hash.endsWith("----")) || false;
const pincodeRequired =
  (document && document.location.hash.endsWith("--")) || false;
const previousUsername =
  (localStorage && localStorage.getItem(USERNAME_KEY)) || "";
...
function chat(state = initialState, action) {
  ...
  case "CHAT_ADD_SUGGESTION":
    ...
    const formattedSuggestion = afterSuggestion.replace(
