...
class ChatHandler {
  ...
  onWsClose = (event) => {
    ...
    this.wsMessageSubject.error(
      new Error("The websocket connection was closed!")
    );
  };
  ...
  onWsError = (event) => {
    ...
    this.wsMessageSubject.error(
      new Error("Error occurred on websocket connection!")
    );
  };
  ...
  onWsMessage = (event) => {
    ...
    if (
      ...
      data.ephemeral.length > 0
    ) {
      Observable.from(data.ephemeral)
        .mergeMap((ephemeral) =>
          this.createDecryptEphemeralObservable({
            ephemeral,
            mID: this.mID,
    ...
    if (data && data.from_server) {
      if (data.from_server.all_messages_deleted) {
        alert(
          "All messages deleted from server! (Refresh this page to remove them from this browser tab.)"
        );
      }
    }
  };
  ...
  resolveIncomingMessageTypeObservable = (message) => {
    if (message.meta.isChatMessage) {
      ...
      const chatMessageObservable = Observable.fromEvent(reader, "loadend")
        .pluck("target", "result")
        .map(JSON.parse)
        .do((contents) => {
      ...
      return Observable.throw(
        new Error("Unrecognized data with type " + message.tags.type)
      );
    }
  };
  ...
  send({ contents = {}, tags, ttl = 0 }) {
    ...
    if (this.ws.readyState !== WebSocket.OPEN) {
      throw new Error(
        "WebSocket connection is initializing and is not open yet!"
      );
    }
